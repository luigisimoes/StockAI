  ...
  return (
    <Sheet open={true} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="w-[800px] max-w-full p-0 flex flex-col [&>button]:hidden" aria-describedby={undefined}>
        ...
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-1.5 text-graphite-400 hover:text-graphite-900 hover:bg-graphite-50 rounded-lg transition-colors z-10"
        ...
        {/* Header */}
        <div className="px-8 pt-6 pb-6 border-b hairline-border">
              ...
              <div className="grid grid-cols-2 gap-6 mt-4">
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <h3 className="text-sm font-bold text-graphite-900">Top signals &amp; weights</h3>
                    <div className="space-y-2.5">
                      {rec.signals.map((sig) => (
                        <div key={sig.label} className="bg-white p-4 hairline-border rounded-xl card-shadow flex items-center justify-between gap-4">
                ...
                <div className="space-y-6">
                  {/* Constraints Checked */}
                  <div className="bg-white p-5 hairline-border rounded-xl card-shadow">
                    <h3 className="text-sm font-bold text-graphite-900 mb-4 flex items-center gap-2">
                  ...
                  <div className="bg-white p-5 hairline-border rounded-xl card-shadow">
                    <h3 className="text-sm font-bold text-graphite-900 mb-3 flex items-center gap-2">
                      <History className="w-4 h-4 text-graphite-400" strokeWidth={1.5} />
                      Past similar decisions
                    </h3>
                    <div className="space-y-2">
                      {rec.pastDecisions.map((pd) => (
                        <div key={pd.date} className="p-4 bg-graphite-50 hairline-border rounded-lg space-y-2">
                ...
                {rec.alternatives.map((alt) => {
                  ...
                  return (
                    <div key={alt.label} className={cn(
                      "p-5 rounded-xl hairline-border card-shadow flex items-center justify-between",
                      isRec ? "bg-indigo-50 border-indigo-200" : "bg-white"
                    )}>
                      <div>
        ...
        {/* Footer Actions */}
        <div className="px-8 py-5 border-t hairline-border bg-white/95 backdrop-blur-md flex items-center gap-6">
          {/* Adjustment Input */}
